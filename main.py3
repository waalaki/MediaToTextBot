Botka hada waxuu soo raaciyaa result ga qoraalo sida **Cleaned Transcript:**  Cleaned Transcript:
ama Summary: halkii uu kaliya result ga kasoo diri lahaa oo an wadan wax intero ah





sidoo kale dhamaan reply massages ka bot ka english kadhig
...
# Saaray: from pymongo import MongoClient
import telebot
...
# Saaray: mongo_client = MongoClient(MONGO_URI)
# Saaray: db = mongo_client[DB_APPNAME]
# Saaray: users_collection = db.users
# Saaray: usage_collection = db.usage
...
# Hadda in-memory storage:
user_lang = {} # {uid: lang_code}
user_mode = {} # {uid: mode_name}
user_transcriptions = {} # {chat_id: {message_id: {"text": transcription_text, "origin": original_msg_id}}}
action_usage = {} # {f"{chat_id}|{message_id}|action": count} - Si loo maamulo xaddidaadda isticmaalka ficillada
user_usage_count = {} # {uid: count} - Si loo maamulo isticmaalka bilaashka ah
# Saaray: action_usage = {}
# Saaray: user_usage_count = {}

# Saaray: set_user_preferences - Hadda waxaa si toos ah loo maamulaa `user_lang` iyo `user_mode`

# Saaray: get_user_preferences
    ...
    for key in gemini_rotator.get_order():
        try:
            ...
            if response.status_code == 200:
                ...
                # Halkan waxaan ku leenahay mark_success si aan u isticmaalno furaha haddii kani yahay mid aan ahayn 403.
                # Waa la hagaajiyay si uu u leexleexo (rotate) haddii aysan 200 ahayn.
                gemini_rotator.mark_failure(key) 
                last_exc = f"HTTP {response.status_code}: {response.text}"
                continue # Iskuday furaha xiga
        except Exception as e:
...
def build_action_keyboard(text_length):
    # Waxaan u baahanahay inaan dhisno badhamada iyada oo aan lahayn message_id ama chat_id maadaama aan ku dari doono fariinta marka la diro.
    # Badhamada callback_data waxaan u adeegsan doonaa kaliya ficilka + calaamad.
    buttons = []
    # Waxaan ku wareejin doonaa message_id-ga callback query-ga.
    buttons.append([InlineKeyboardButton("⭐️Clean transcript", callback_data=f"clean|")])
    if text_length > 1000:
...
async def action_callback_query(client, callback_query: CallbackQuery):
    ...
    action, _ = callback_query.data.split("|") # Waxaan iska indha tiraynaa qaybta labaad ee faaruqa ah.
    chat_id = callback_query.message.chat.id
    message_id = callback_query.message.id
    uid = callback_query.from_user.id
    
    # Hubi in qoraalka uu ku jiro xusuusta
    transcription_data = user_transcriptions.get(chat_id, {}).get(message_id)
    if not transcription_data:
        await callback_query.answer("Qoraalka la qoray lama helin. Fariinta mar kale dir.", show_alert=True)
        return
    
    original_text = transcription_data["text"]
    
    # Maamul xaddidaadda isticmaalka
    key = f"{chat_id}|{message_id}|{action}"
    usage_count = action_usage.get(key, 0)
    
    if usage_count >= 1: # U ogolow hal isticmaal oo kaliya ficil kasta oo ku jira fariin kasta.
        await callback_query.answer(f"Waxa aad isticmaashay {action} fariintan mar hore.", show_alert=True)
        return
        
    await callback_query.answer("Waan shaqaynayaa... Fadlan sug.", show_alert=False)
    await client.send_chat_action(chat_id, ChatAction.TYPING)
    
    instruction = ""
    if action == "clean":
        instruction = "Clean up the provided text. Fix any punctuation, capitalization, and grammatical errors. Ensure the output is a professional, polished version of the original text. Do not add any introductory or concluding phrases."
        action_name = "Cleaned Transcript"
    elif action == "summarize":
        instruction = "Summarize the provided text concisely. The summary should capture all main points and be easy to read. Do not add any introductory or concluding phrases."
        action_name = "Summary"
    else:
        await callback_query.answer("Ficil aan la aqoon.", show_alert=True)
        return
    ...
    try:
        ...
        if processed_text.startswith("Error:"):
            await callback_query.message.reply_text(f"❌ **{action_name} Error:** {processed_text}", reply_to_message_id=transcription_data["origin"])
            return
            
        action_usage[key] = usage_count + 1 # Kordhi tirada isticmaalka
        
        # Cusboonaysii badhamada
        ...
        for row in keyboard.inline_keyboard:
            ...
            for button in row:
                if button.callback_data and button.callback_data.startswith(f"{action}|"):
                    # Markaa badhanka hadda jira (action) wuu dansan yahay ama wuu isbeddelayaa
                    new_row.append(InlineKeyboardButton(f"✅ {button.text}", callback_data="used")) # Samee used
                else:
        ...
        try:
            ...
            pass # Mararka qaarkood fariinta lama beddeli karo
        
        # Dir natiijada
        final_response = f"**{action_name}:**\n\n{processed_text}"
        
        if len(final_response) > MAX_MESSAGE_CHUNK:
            # Haddii natiijadu dheer tahay, u dir Text File ama Split messages, iyadoo la raacayo qaabka isticmaalaha
            mode = get_user_mode(uid, "📄 Text File")
            
            if mode == "💬 Split messages":
                await callback_query.message.reply_text(f"**{action_name}** - Fariin dheer: Qayb ahaan loo diray.")
                ...
                file_name = os.path.join(DOWNLOADS_DIR, f"{action_name.replace(' ', '_')}.txt")
                with open(file_name, "w", encoding="utf-8") as f:
                    f.write(final_response)
                await client.send_chat_action(chat_id, ChatAction.UPLOAD_DOCUMENT)
                await client.send_document(chat_id, file_name, caption=f"Open this file and copy the **{action_name}** 👍", reply_to_message_id=transcription_data["origin"])
                os.remove(file_name)
        else:
            await client.send_message(chat_id, final_response, reply_to_message_id=transcription_data["origin"])

    except RuntimeError as e:
        await callback_query.message.reply_text(f"❌ **{action_name} Fashilmay:** {e}", reply_to_message_id=transcription_data["origin"])
    except Exception as e:
        logging.error(f"Action callback error: {e}")
        await callback_query.message.reply_text(f"❌ **Fashil aan la filayn oo dhacay:** {e}", reply_to_message_id=transcription_data["origin"])
...
async def handle_media(client, message: Message):
    ...
    if sent_message:
        try:
            ...
            # Hadda waxaan ku kaydinaynaa qoraalka la qoray message_id si aan u helno badhamada
