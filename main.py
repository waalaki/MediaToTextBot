...
def acquire_user_lock(uid):
    with locks_lock:
        if uid not in per_user_locks:
            per_user_locks[uid] = threading.Lock()
        lock = per_user_locks[uid]
    acquired = lock.acquire(blocking=False)
    return acquired

def release_user_lock(uid):
    with locks_lock:
        lock = per_user_locks.get(uid)
    if not lock:
        return
    try:
        if lock.locked():
        ...
        pass
    with locks_lock:
        l = per_user_locks.get(uid)
        if l and not l.locked():
            try:
                del per_user_locks[uid]
            except:
                pass
...
def send_long_text(chat_id, text, reply_id, uid=None, action="Transcript"):
    mode = get_user_mode(uid or 0)
