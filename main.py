...
def format_hms(secs):
    ...
    m = (secs % 3600) // 60
    s = secs % 60
    return f"{h} saac {m} daqiiqo {s} ilbiriqsi"
...
def setkey_cmd(message):
    ...
    bot.reply_to(message, "Key-ga waa la kaydiyey. Waxaad haysataa 19 codsi 24 saac gudahood. Hadda waad isticmaali kartaa bot-ka.")
...
def text_handler(message):
    ...
    if message.text.startswith("/getcount"):
        ...
        if cnt >= DAILY_LIMIT:
            bot.reply_to(message, f"Waxaad gaartay limit-ka codsiyada ee {DAILY_LIMIT}. Inta ka hartay: {format_hms(rem)}.")
        else:
            bot.reply_to(message, f"Hadda waxaad isticmaashay {cnt} codsi. Waqtiga hadhay ee daaqadda: {format_hms(rem)}. Limit-ka waa {DAILY_LIMIT}.")
        return
...
def process_text_action(call, origin_msg_id, log_action, prompt_instr):
    ...
    try:
        ...
        if msg == "API_KEY_MISSING":
            ...
            bot.send_message(chat_id, f"Waxaad gaartay limit-ka codsiyada ee {DAILY_LIMIT}. Inta ka hartay: {format_hms(secs)}.")
        else:
    ...
    try:
        ...
        try:
            ...
            if em.startswith("API_DAILY_LIMIT_REACHED"):
                ...
                bot.reply_to(message, f"Waxaad gaartay limit-ka codsiyada ee {DAILY_LIMIT}. Inta ka hartay: {format_hms(secs)}.")
                return
            raise
        if not text:
