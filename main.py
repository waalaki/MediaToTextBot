        ...
        if "choices" in j and isinstance(j["choices"], list) and j["choices"]:
            ...
            if isinstance(c, dict):
                ...
                if "message" in c and isinstance(c["message"], dict):
                    mc = c["message"]
                    if "content" in mc and isinstance(mc["content"], str):
                        return mc["content"]
...
def ask_groq_with_key(text, instruction, key):
    ...
    payload = {"model": GROQ_CHAT_MODEL, "messages": [{"role": "user", "content": f"{instruction}\n\n{text}"}]}
        ...
        if "choices" in j and isinstance(j["choices"], list) and j["choices"]:
            ...
            if isinstance(choice, dict):
                ...
                if "text" in choice and isinstance(choice["text"], str):
                    return choice["text"]
                if "message" in choice and isinstance(choice["message"], str):
...
def lang_cb(call):
    ...
    try:
        text = upload_and_transcribe_groq(file_path, language=code)
        if not text:
...
def handle_media(message):
    ...
    try:
        ...
        if not lang:
            ...
            return
        text = upload_and_transcribe_groq(file_path, language=lang)
        if not text:
