...
REQUIRED_CHANNEL = os.environ.get("REQUIRED_CHANNEL", "") or ""
...
def transcribe_local_audio(file_path, language_code="en-US"):
    ...
    try:
        ...
        with sr.AudioFile(wav_path) as source:
            while True:
                audio_data = recognizer.record(source, duration=MAX_CHUNK_DURATION)
...
def ensure_joined(message):
    if not REQUIRED_CHANNEL or REQUIRED_CHANNEL.strip() == "":
        return True
    try:
        chat_identifier = REQUIRED_CHANNEL
        try:
            chat_identifier = int(REQUIRED_CHANNEL)
        except Exception:
            chat_identifier = REQUIRED_CHANNEL
        member = bot.get_chat_member(chat_identifier, message.from_user.id)
        if getattr(member, "status", "") in ['member', 'administrator', 'creator']:
            return True
    except Exception:
        pass
    clean = REQUIRED_CHANNEL.replace("@", "").strip()
