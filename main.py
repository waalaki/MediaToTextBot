...
def process_text_action(call, origin_msg_id, log_action, prompt_instr):
    ...
    try:
        ...
        logging.exception("Error in process_text_action")
        try:
            bot.answer_callback_query(call.id, "Server error please report 👉🏻 @orlaki", show_alert=True)
        except:
            try:
                bot.send_message(chat_id, "Server error please report 👉🏻 @orlaki")
            except:
                pass
...
def handle_media(message):
    ...
    try:
        ...
        logging.exception("Error handling media")
        try:
            if progress_msg:
                try:
                    bot.delete_message(progress_msg.chat.id, progress_msg.message_id)
            ...
            pass
        try:
            bot.reply_to(message, "Server error please report 👉🏻 @orlaki")
        except:
            pass
        try:
