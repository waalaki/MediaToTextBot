...
assembly_rotator = KeyRotator(ASSEMBLYAI_API_KEYS)
...
def execute_assembly_action(action_callback):
    ...
    for _ in range(len(assembly_rotator.keys) + 1):
        ...
        if not key: raise RuntimeError("No AssemblyAI keys available")
        try:
            result = action_callback(key)
...
def transcribe_with_assemblyai(file_path):
    def perform_assembly_transcribe(key):
    ...
    return execute_assembly_action(perform_assembly_transcribe)

def upload_and_transcribe_hybrid(file_path: str) -> str:
    try:
        ...
        logging.warning(f"Gemini failed, switching to AssemblyAI: {e}")
        if not ASSEMBLYAI_API_KEYS:
            raise e
        return transcribe_with_assemblyai(file_path)
...
def handle_media(message):
    ...
    try:
        ...
        text = upload_and_transcribe_hybrid(file_path)
