...
def transcribe_via_url_groq(audio_url, language=None):
    if not groq_rotator.keys:
        raise RuntimeError("Groq key(s) not configured")
    def perform_all_steps(key):
        tmp = tempfile.NamedTemporaryFile(delete=False, dir=DOWNLOADS_DIR)
        tmp_path = tmp.name
        tmp.close()
...
def handle_media(message):
    ...
    try:
        ...
        text = transcribe_via_url_groq(telegram_file_url, language=lang)
        if not text:
