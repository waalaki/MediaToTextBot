...
def lang_cb(call):
    ...
    if origin != "file":
        ...
        try:
            ...
            txt = sanitize_error_text(tb)
            try:
                bot.send_message(call.message.chat.id, f"Error: {txt}")
            except Exception:
...
def process_text_action(call, origin_msg_id, log_action, prompt_instr):
    ...
    try:
        ...
        txt = sanitize_error_text(tb)
        try:
            bot.send_message(chat_id, f"Error: {txt}")
        except Exception:
            logging.exception("Failed to send error to user")
...
def handle_media(message):
    ...
    try:
        ...
        logging.exception("Error while handling media")
        tb = traceback.format_exc()
        txt = sanitize_error_text(tb)
        try:
            bot.send_message(message.chat.id, f"Processing error:\n{txt}")
        except Exception:
            logging.exception("Failed to send processing error to user")
    finally:
        for fpath in locals().get("created_files", []):
            try:
...
def _process_webhook_update(raw):
    try:
        ...
        txt = sanitize_error_text(tb)
        try:
            bot.send_message(ADMIN_CHAT_ID, f"Webhook processing error:\n{txt}")
        except Exception:
            logging.exception("Failed to send webhook error to admin")
...
if __name__ == "__main__":
    if WEBHOOK_URL:
        try:
            ...
            txt = sanitize_error_text(tb)
            try:
                bot.send_message(ADMIN_CHAT_ID, f"Startup error:\n{txt}")
            except Exception:
                logging.exception("Failed to send startup error to admin")
    else:
