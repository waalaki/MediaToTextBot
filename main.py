...
def execute_gemini_action(action_callback, uid=None):
    ...
    for _ in range(len(gemini_rotator.keys) + 1):
        ...
        try:
            ...
            last_exc = e
            logging.warning(f"Gemini error with key {str(key)[:6]}: {e}")
...
def handle_media(message):
    ...
    if get_daily_count(message.from_user.id) >= DAILY_LIMIT:
        ...
        return
    if not GEMINI_API_KEYS and message.from_user.id not in user_api_keys:
        bot.reply_to(message, "Please set your Gemini API key first with /setkey YOUR_GEMINI_API_KEY")
        return
