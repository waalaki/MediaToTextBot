...
def init_db():
    global client_mongo, db, users_col, user_mode, user_selected_lang
    try:
        ...
        cursor = users_col.find({}, {"user_id": 1, "mode": 1, "lang": 1})
        for doc in cursor:
            uid = int(doc["user_id"])
            if "mode" in doc and isinstance(doc["mode"], str) and doc["mode"].strip():
                user_mode[uid] = doc["mode"]
            if "lang" in doc and isinstance(doc["lang"], str) and doc["lang"].strip():
                user_selected_lang[uid] = doc["lang"]
    except Exception as e:
        logging.warning("MongoDB init failed: %s", e)

def set_user_pref(uid, mode=None, lang=None):
    try:
        if mode is not None:
            user_mode[uid] = mode
        if lang is not None:
            user_selected_lang[uid] = lang
        if users_col is not None:
            ...
            if update:
                ...
                users_col.update_one({"user_id": uid}, {"$set": update}, upsert=True)
    except Exception as e:
        logging.warning("Failed to set user pref: %s", e)
...
def handle_media(message):
    ...
    try:
        ...
        with open(file_path, 'wb') as f:
            f.write(downloaded)
        lang = user_selected_lang.get(message.from_user.id)
        if not lang:
        ...
        bot.reply_to(message, "❌ Error: %s" % e)
        try:
