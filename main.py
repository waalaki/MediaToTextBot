...
# --- GEMINI REST API FUNCTIONS (FIXED) ---
...
def upload_and_transcribe_gemini(file_path: str, lang_code: str = "en") -> str:
    ...
    for key in gemini_rotator.get_order():
        try:
            # --- 1. UPLOAD (REST API) ---
            upload_url = f"https://generativelanguage.googleapis.com/v1beta/files?key={key}"
            # Headers for upload can be simple, requests handles content-type for files
            upload_headers = {"X-Goog-Api-Key": key}
            
            logging.info(f"Attempting file upload: {file_path} with key {key[:4]}...")
            
            with open(file_path, 'rb') as f:
                # Use multipart/form-data via requests 'files' parameter
                upload_files = {
            ...
            # --- JSON PARSING FIX IS HERE ---
            upload_data = upload_response.json()
            logging.info(f"Upload Response JSON: {upload_data}") # Log for debugging
            
            # Gemini returns {"file": {"name": "files/..."}} usually
            if "file" in upload_data and "name" in upload_data["file"]:
                ...
                raise RuntimeError("Uploaded file name missing from response JSON.")
