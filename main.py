...
def send_welcome(message):
    if ensure_joined(message):
        welcome_text = (
            "Salaam!\n"
            "Send voice, audio, video or document to transcribe.\n"
            "Developer: @orlaki"
        )
...
def mode_cb(call):
    ...
    try:
        bot.edit_message_text("you chose: %s" % mode, call.message.chat.id, call.message.message_id, reply_markup=None)
    except:
        pass
    try:
        bot.answer_callback_query(call.id, "Mode set to: %s" % mode)
    except:
        pass
...
def lang_command(message):
    if ensure_joined(message):
        kb = build_lang_keyboard("file")
        bot.reply_to(message, "Select the language:", reply_markup=kb)
...
def lang_cb(call):
    ...
    if origin != "file":
        try:
            ...
            pass
        process_text_action(call, origin, "Translate to %s" % lbl, "Translate this text into %s. Return only the translated text." % lbl)
        return
    try:
    ...
    try:
        bot.answer_callback_query(call.id, "Language set: %s" % lbl)
    except:
        pass

@bot.callback_query_handler(func=lambda c: c.data.startswith('translate_menu|') or c.data.startswith('summarize|') or c.data.startswith('summarize_menu|') or c.data.startswith('summopt|'))
def generic_action_cb(call):
    data = call.data
    if data.startswith('translate_menu|'):
        try:
            bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id, reply_markup=build_lang_keyboard(str(call.message.message_id)))
        except:
            pass
        return
    if data.startswith('summarize|') or data.startswith('summarize_menu|'):
        try:
            bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id, reply_markup=None)
        except:
            pass
        process_text_action(call, str(call.message.message_id), "Summarize", "Summarize this in original language.")
        return
    if data.startswith('summopt|'):
        parts = data.split("|")
        if len(parts) != 3:
            try:
        ...
        try:
            ...
            pass
        if style == "Short":
            prompt = "Summarize this text in the original language in 1-2 concise sentences. Return only the summary."
        elif style == "Detailed":
            prompt = "Summarize this text in the original language in a detailed paragraph preserving key points. Return only the summary."
        else:
            prompt = "Summarize this text in the original language as a bulleted list of main points. Return only the summary."
...
def webhook():
    if request.headers.get('content-type') and 'application/json' in request.headers.get('content-type'):
        ...
        try:
            ...
            try:
                update_queue.put(data, timeout=1)
            except:
                pass
        return '', 200
    abort(403)
...
def handle_media(message):
    ...
    if getattr(media, 'file_size', 0) > MAX_UPLOAD_SIZE:
        bot.reply_to(message, "Send a file less than %sMB" % MAX_UPLOAD_MB)
        return
    try:
    ...
    try:
        ...
        try:
            ...
            pass
        logging.exception("media handling error: %s", e)
        try:
...
if __name__ == "__main__":
    if WEBHOOK_URL:
        ...
        try:
            ...
            for _ in workers:
                try:
                    update_queue.put(None, timeout=1)
                except:
                    try:
