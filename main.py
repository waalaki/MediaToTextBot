...
def lang_cb(call):
    ...
    try:
        text = transcribe_file_with_sr(file_path, language=code)
        if not text:
...
def convert_to_wav(input_path):
    ...
    out = os.path.join(DOWNLOADS_DIR, f"{base}.wav")
    cmd = ["ffmpeg", "-y", "-i", input_path, "-ac", "1", "-ar", "16000", "-vn", "-c:a", "pcm_s16le", out]
...
def split_wav_to_chunks(wav_path):
    ...
    out_pattern = os.path.join(DOWNLOADS_DIR, f"{base}_chunk_%03d.wav")
    cmd = ["ffmpeg", "-y", "-i", wav_path, "-f", "segment", "-segment_time", "60", "-ac", "1", "-ar", "8000", "-c:a", "pcm_s16le", out_pattern]
    subprocess.run(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True)
    files = sorted([os.path.join(DOWNLOADS_DIR, f) for f in os.listdir(DOWNLOADS_DIR) if f.startswith(base + "_chunk_") and f.endswith(".wav")])
    return files

def transcribe_file_with_sr(input_path, language=None):
    try:
        wav = input_path if input_path.lower().endswith(".wav") else convert_to_wav(input_path)
    except Exception as e:
        raise RuntimeError(f"Conversion to WAV failed: {e}")
    chunks = split_wav_to_chunks(wav)
    r = sr.Recognizer()
    texts = []
    for ch in chunks:
        ...
        try:
            ...
            raise RuntimeError(f"Recognition request failed: {e}")
        texts.append(txt)
...
def handle_media(message):
    ...
    try:
        ...
        if not lang:
            ...
            bot.reply_to(message, "Select the language spoken in your audio or video:", reply_markup=kb)
            return
        text = transcribe_file_with_sr(file_path, language=lang)
        if not text:
