...
# Dejinta deegaanka iyo xuduudaha
...
# Waxaan dejineynaa xaddiga jarjarka (chunk) 55 ilbidhiqsi sida aad codsatay, si aan uga gaabino 1 daqiiqo (60s).
MAX_CHUNK_SECONDS = int(os.environ.get("MAX_CHUNK_SECONDS", "55")) 
...
# Qaybta KeyRotator, Gemini, iyo Langs (lama taaban)
class KeyRotator:
...
def transcribe_file_with_speech_recognition(file_path, language=None):
    """
    Waxay ku shaqaysaa Transcription oo ku salaysan faylka.
    Waxay isticmaashaa Vosk haddii shuruuduhu buuxsamaan, haddii kale waxay isticmaashaa Google Speech Recognition.
    Waxa la jarjarayaa (chunking) audio-ga haddii uu ka dheeraado MAX_CHUNK_SECONDS si loo waafajiyo Google API xadigeeda.
    """
    audio = AudioSegment.from_file(file_path)
    duration_ms = len(audio)
    # Waxay u beddeshaa 16kHz, mono kanaal (shuruudda Vosk iyo Google)
    audio = audio.set_frame_rate(16000).set_channels(1)
    
    # Haddii aan la adeegsaneynin Vosk ama uu audio-gu ka dheer yahay xadiga chunk-ga
    if not (language == "en" and vosk_model is not None and duration_ms <= MAX_CHUNK_MS):
        
        # Haddii uu audio-gu ka dheer yahay xadiga chunk-ga (MAX_CHUNK_MS), waa la jarjarayaa (chunking)
        if duration_ms > MAX_CHUNK_MS:
            logging.info(f"Audio-gu waa {duration_ms/1000}s, wuu ka dheer yahay {MAX_CHUNK_SECONDS}s. Waxaa la sameynayaa chunking.")
            full_transcript = []
            
            # Waxay jarjaraysaa qaybo (chunks) ah MAX_CHUNK_MS
            for i in range(0, duration_ms, MAX_CHUNK_MS):
                chunk = audio[i:i + MAX_CHUNK_MS]
                
                # Waxay ku kaydineysaa qaybta fayl WAV ku meel gaar ah
                tmp_chunk = tempfile.NamedTemporaryFile(suffix=f"_{uuid.uuid4().hex}_chunk.wav", delete=False, dir=DOWNLOADS_DIR)
                tmp_chunk.close()
                ...
                try:
                    ...
                    logging.warning(f"Google Speech Recognition ma fahmin qeybta {i}ms")
                    full_transcript.append("") 
                except sr.RequestError as e:
                    raise RuntimeError(f"Google Speech Recognition way fashilantay; hubi inaad online tahay: {e}")
                finally:
                    try:
        ...
        # Haddii uu audio-gu ka gaaban yahay xadiga chunk-ga (laakin aan la adeegsan Vosk)
        else:
    ...
    # Haddii ay buuxdo shuruudaha Vosk (Ingiriis & xadiga chunk-ga)
    else:
        logging.info("Waxaa la adeegsanayaa Vosk.")
        tmp = tempfile.NamedTemporaryFile(suffix=f"_{uuid.uuid4().hex}.wav", delete=False, dir=DOWNLOADS_DIR)
        tmp.close()
        audio.export(tmp.name, format="wav")
        try:
