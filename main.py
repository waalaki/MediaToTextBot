...
.card{background:#fff;padding:32px 48px;border-radius:12px;box-shadow:0 6px 18px rgba(0,0,0,0.08);text-align:center}
.status-dot{width:14px;height:14px;border-radius:50%;display:inline-block;margin-right:10px;vertical-align:middle;background:#28a745;box-shadow:0 0 8px rgba(40,167,69,0.28)}
...
<div class="container">
  <div class="card">
    <div><span class="status-dot"></span><strong>I'm online</strong></div>
    <h1>Bot status</h1>
    <p>Bot is running and accepting requests</p>
    <div class="footer">Port: %d</div>
  </div>
</div>
</body>
</html>""" % PORT
    ...
    try:
        ...
        thread.start()
        logging.info("Web status server started on port %d", PORT)
    except Exception as e:
