...
APIFREE_URL = os.environ.get("APIFREE_URL", "https://apifreellm.com/api/chat")
...
def apifreellm_call(payload):
    headers = {"Content-Type": "application/json"}
    resp = requests.post(APIFREE_URL, headers=headers, json=payload, timeout=REQUEST_TIMEOUT)
...
def ask_apifreellm(text, instruction):
    payload = {"message": f"{instruction}\n\n{text}"}
    data = apifreellm_call(payload)
...
def transcribe_media_apifreellm(file_url, mime_type, language_code):
...
def send_welcome(message):
    if ensure_joined(message):
        welcome_text = (
            ...
            "• audio file\n"
            "• video\n"
            "• to transcribe using the free LLM API\n\n"
...
def lang_cb(call):
    ...
    try:
        text = transcribe_media_apifreellm(file_url, mime_type, code)
        if not text:
...
def process_text_action(call, origin_msg_id, log_action, prompt_instr):
    ...
    bot.send_chat_action(chat_id, 'typing')
    try:
        res = ask_apifreellm(text, prompt_instr)
...
def handle_media(message):
    ...
    try:
        ...
        if not lang:
            ...
            bot.reply_to(message, "Select the language spoken in your audio or video:", reply_markup=kb)
            return
        text = transcribe_media_apifreellm(telegram_file_url, mime_type, lang)
        if not text:
