...
def user_friendly_error(chat_id):
    try:
        bot.send_message(chat_id, "Sorry, something went wrong there. Try again.")
    except:
        pass

def admin_report(error, context="", message_obj=None):
    try:
    ...
    try:
        user_info = ""
        if message_obj and getattr(message_obj, "from_user", None):
            u = message_obj.from_user
            uname = getattr(u, "username", None)
            if uname:
                user_info = f"User @{uname} id={u.id}"
            else:
                name = " ".join(filter(None, [getattr(u, "first_name", ""), getattr(u, "last_name", "")])).strip()
                user_info = f"User {name} id={u.id}"
        ...
        logging.exception("Failed to report to admin")
...
def lang_cb(call):
    ...
    if origin != "file":
        ...
        try:
            ...
            admin_report(e, "lang_cb non-file processing", call.message)
            user_friendly_error(call.message.chat.id)
        return
    try:
        try:
    ...
    try:
        ...
        admin_report(e, "lang_cb transcription", orig_msg)
        user_friendly_error(chat_id)
    finally:
        try:
...
def action_cb(call):
    ...
    if action == "translate_menu":
        ...
        try:
            ...
            admin_report(e, "action_cb summarize", call.message)
            user_friendly_error(call.message.chat.id)
...
def process_text_action(call, origin_msg_id, log_action, prompt_instr):
    ...
    try:
        ...
        admin_report(e, "process_text_action", call.message)
        user_friendly_error(chat_id)
    ...
    try:
        ...
        try:
            ...
            if uname:
                ...
                sender = " ".join(filter(None, [getattr(u, 'first_name', ''), getattr(u, 'last_name', '')])).strip() or str(getattr(u, 'id', 'unknown'))
            bot.send_message(ADMIN_ID, f"File received from {sender}")
        except Exception as e:
            logging.exception("Failed to notify admin about file")
        ...
        admin_report(e, "handle_media", message)
        user_friendly_error(message.chat.id)
        try:
...
if __name__ == "__main__":
    if WEBHOOK_URL:
        ...
        try:
            ...
            admin_report(e, "setting webhook")
