...
def increment_daily(uid):
    ...
    entry = user_daily.get(uid)
    if not entry or entry.get("date") != today:
        user_daily[uid] = {"date": today, "count": 0}
        entry = user_daily[uid]
...
def execute_gemini_action(action_callback, uid=None):
    if not uid or uid not in user_api_keys:
        raise RuntimeError("You must set your Gemini API key first using /setkey YOUR_GEMINI_API_KEY")
    key = user_api_keys[uid]
    try:
        result = action_callback(key)
        return result
    except Exception as e:
        try:
            del user_api_keys[uid]
        except:
            pass
        raise RuntimeError(f"Your provided Gemini API key failed and was removed. Error: {e}")
...
def upload_and_transcribe_gemini(file_path: str, uid) -> str:
    if get_daily_count(uid) >= DAILY_LIMIT:
        raise RuntimeError(f"Daily request limit reached ({DAILY_LIMIT}). Try again tomorrow.")
        ...
        try:
            ...
            if uploaded_name:
                try:
                    ...
                    pass
    res = execute_gemini_action(perform_upload_and_transcribe, uid=uid)
    increment_daily(uid)
    return res
...
def ask_gemini(text, instruction, uid):
    ...
    def perform_text_query(key):
        ...
        data = gemini_api_call(f"models/{GEMINI_MODEL}:generateContent", payload, key, headers={"Content-Type": "application/json"})
        return data["candidates"][0]["content"]["parts"][0]["text"]
    res = execute_gemini_action(perform_text_query, uid=uid)
    increment_daily(uid)
    return res
...
def send_welcome(message):
    if ensure_joined(message):
        welcome_text = (
            ...
            "If you want the bot to use your Gemini API key only, set it with:\n"
            "/setkey YOUR_GEMINI_API_KEY\n\n"
            f"Daily request limit per user is {DAILY_LIMIT} requests."
        )
        bot.reply_to(message, welcome_text, parse_mode="Markdown")

@bot.message_handler(commands=['setkey'])
def set_key_cmd(message):
    parts = (message.text or "").strip().split()
    if len(parts) < 2:
        bot.reply_to(message, "Use: /setkey YOUR_GEMINI_API_KEY")
        return
    key = parts[1].strip()
    if not key:
        bot.reply_to(message, "Key invalid. Provide a non-empty key.")
        return
    user_api_keys[message.from_user.id] = key
    bot.reply_to(message, "Saved your Gemini API key. It will be used only for your requests.")
...
def process_text_action(call, origin_msg_id, log_action, prompt_instr):
    ...
    if "Summarize" in log_action and action_usage.get(key, 0) >= 1:
        ...
        return
    if get_daily_count(call.from_user.id) >= DAILY_LIMIT:
        bot.answer_callback_query(call.id, f"Daily limit reached ({DAILY_LIMIT}).", show_alert=True)
        return
...
def handle_media(message):
    ...
    if getattr(media, 'file_size', 0) > MAX_UPLOAD_SIZE:
        ...
        return
    if get_daily_count(message.from_user.id) >= DAILY_LIMIT:
        bot.reply_to(message, f"Daily limit reached ({DAILY_LIMIT}). Try again tomorrow.")
        return
    if message.from_user.id not in user_api_keys:
        bot.reply_to(message, "Please set your Gemini API key first with /setkey YOUR_GEMINI_API_KEY. No shared keys are allowed.")
        return
