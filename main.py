...
VOSK_EN_MODEL_PATH = os.environ.get("VOSK_EN_MODEL_PATH", "./vosk-model-small-en-us-0.15")
VOSK_EN_ZIP_URL = os.environ.get("VOSK_EN_ZIP_URL", "https://alphacephei.com/vosk/models/vosk-model-small-en-us-0.15.zip")
VOSK_AR_MODEL_PATH = os.environ.get("VOSK_AR_MODEL_PATH", "./vosk-model-ar-mgb2-0.4")
VOSK_AR_ZIP_URL = os.environ.get("VOSK_AR_ZIP_URL", "https://alphacephei.com/vosk/models/vosk-model-ar-mgb2-0.4.zip")
...
def ensure_vosk_model(model_path, zip_url):
    if os.path.isdir(model_path):
        return
    logging.info(f"Downloading Vosk model from {zip_url} to {model_path}...")
    zip_path = os.path.join(DOWNLOADS_DIR, f"{os.path.basename(model_path)}.zip")
    try:
        urllib.request.urlretrieve(zip_url, zip_path)
        with zipfile.ZipFile(zip_path, 'r') as z:
            z.extractall(".")
        expected_folder_name = os.path.basename(zip_url).replace(".zip", "")
        if os.path.isdir(expected_folder_name) and expected_folder_name != os.path.basename(model_path):
            if os.path.exists(model_path):
                shutil.rmtree(model_path)
            shutil.move(expected_folder_name, model_path)
        elif not os.path.isdir(model_path) and os.path.isdir(expected_folder_name):
            shutil.move(expected_folder_name, model_path)
        os.remove(zip_path)
        logging.info(f"Vosk model downloaded and extracted to {model_path}")
    except Exception as e:
        logging.error(f"Failed to download or extract Vosk model from {zip_url}: {e}")
...
try:
    ensure_vosk_model(VOSK_EN_MODEL_PATH, VOSK_EN_ZIP_URL)
    vosk_models["en"] = Model(VOSK_EN_MODEL_PATH)
except Exception as e:
    logging.warning(f"Vosk English model load failed: {e}")

try:
    ensure_vosk_model(VOSK_AR_MODEL_PATH, VOSK_AR_ZIP_URL)
    vosk_models["ar"] = Model(VOSK_AR_MODEL_PATH)
except Exception as e:
    logging.warning(f"Vosk Arabic model load failed: {e}")

def transcribe_with_vosk(wav_path, lang_code):
    model = vosk_models.get(lang_code)
    if not model:
        raise ValueError(f"Vosk model not loaded for language: {lang_code}")
    
    wf = wave.open(wav_path, "rb")
    if wf.getnchannels() != 1 or wf.getframerate() != 16000:
        wf.close()
        raise ValueError("Vosk requires mono, 16000 Hz WAV audio.")
...
def transcribe_file_with_speech_recognition(file_path, language=None):
    ...
    try:
        if language in vosk_models:
            logging.info(f"Using Vosk for transcription in {language}")
            text = transcribe_with_vosk(tmp.name, language)
        else:
            logging.info(f"Using Google Speech Recognition for transcription in {language}")
...
def handle_media(message):
    ...
    try:
        ...
        if not text:
            raise ValueError("Empty response or audio was unintelligible")
