...
def forward_key_video(chat_id):
    """
    Function-ka cusub ee Forward-gareynaya video-ga.
    """
    bot.send_message(chat_id, "Please send your Gemini API key first.")
    try:
        # Forward-garee fariinta video-ga ah
        bot.forward_message(
            chat_id=chat_id,
            from_chat_id=KEY_PROMPT_CHANNEL_ID,
            message_id=KEY_PROMPT_VIDEO_ID
        )
    except Exception as e:
        logging.error(f"Failed to forward key video: {e}")
        # Haddii uu Forward-ga ku guuldareysto, dir fariin kale si uu user-ka u fahmo
        bot.send_message(chat_id, f"Error ku yimid Forward-ga video-ga. Fadlan si caadi ah u soo dir furahaaga: /setkey YOUR_GEMINI_KEY")
...
def process_text_action(call, origin_msg_id, log_action, prompt_instr):
    ...
    try:
        ...
        if msg == "API_KEY_MISSING":
            # Bedelka ugu horeeya: Forward-garee halkii aad qoraal ka diri lahayd
            forward_key_video(chat_id)
    ...
    try:
        ...
        try:
            ...
            if em == "API_KEY_MISSING":
                # Bedelka labaad: Forward-garee halkii aad qoraal ka diri lahayd
                forward_key_video(message.chat.id)
                return
