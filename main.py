...
try:
    ...
    logging.warning("MongoDB setup issue: %s", e)
...
def upload_and_transcribe_gemini(file_path: str, key: str, uid: int) -> str:
    ...
    try:
        ...
        for p in [file_path, optimized_path]:
            if os.path.exists(p):
                try: os.remove(p)
                except: pass
...
async def handle_media(client, message):
    if not await ensure_joined(client, message):
        return
    media = message.voice or message.audio or message.video or (message.document if message.document and message.document.mime_type and ("audio" in message.document.mime_type or "video" in message.document.mime_type) else None)
    if not media:
        return
    ...
    if len(text) > MAX_MESSAGE_CHUNK:
        if mode == "Split messages":
            ...
            with open(fname, "w", encoding="utf-8") as f:
                f.write(text)
            sent = await client.send_document(chat_id, fname, caption="Open this file for the full text 👍", reply_to_message_id=reply_id)
            try: os.remove(fname)
            except: pass
            return sent
    ...
    <html>
      <head>
        ...
        <style>
          ...
          .card {background:#fff;padding:30px;border-radius:15px;box-shadow:0 4px 15px rgba(0,0,0,0.1);text-align:center}
      ...
      <body>
        <div class="card">
          ...
          <div style="margin-top:10px">Uptime: {{uptime}}s</div>
        </div>
      </body>
    </html>
    """
