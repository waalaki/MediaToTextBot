...
def transcribe_audio_sr(file_path, language="en-US"):
    try:
        ...
        for i, chunk in enumerate(chunks):
            chunk_filename = f"{file_path}_chunk_{i}.wav"
...
def lang_cb(call):
    ...
    try:
        text = transcribe_audio_sr(file_path, language=code)
        if not text:
            raise ValueError("Could not recognize audio or empty.")
...
def handle_media(message):
    ...
    try:
        ...
        if not lang:
            ...
            bot.reply_to(message, "Select the language spoken in your audio or video:", reply_markup=kb)
            return
        text = transcribe_audio_sr(file_path, language=lang)
        if not text:
            raise ValueError("Could not recognize audio or empty.")
