...
GROQ_KEYS = os.environ.get("GROQ_KEYS", GROQ_KEY)
...
def _progress_updater_thread(chat_id, progress_message_id, done_event, label="Processing"):
    percent = 0
    bars = 12
    bar_empty = "░"
    bar_full = "█"
...
def transcribe_media_groq(file_url, mime_type, language_code):
    ...
    try:
        ...
        if not ext:
            parsed = file_url.split("?")[0]
            if "." in parsed:
                ext = "." + parsed.split(".")[-1]
        if not ext:
            ext = ".bin"
        fd, tmp_in = tempfile.mkstemp(suffix=ext)
        os.close(fd)
        with open(tmp_in, "wb") as f:
            f.write(file_content)
        need_mp3 = not mime_type in ("audio/mpeg", "audio/mp3") and not tmp_in.lower().endswith(".mp3")
        use_path = tmp_in
        use_mime = mime_type or "application/octet-stream"
        if need_mp3:
            fd2, tmp_out = tempfile.mkstemp(suffix=".mp3")
            os.close(fd2)
            try:
                subprocess.run(["ffmpeg", "-y", "-i", tmp_in, "-ar", "16000", "-ac", "1", "-b:a", "48k", "-f", "mp3", tmp_out], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=60)
                use_path = tmp_out
                use_mime = "audio/mpeg"
            except Exception:
                use_path = tmp_in
        ...
        def perform(key):
            with open(use_path, "rb") as audio_file:
                files = {'file': ('audio.mp3', audio_file, use_mime)}
                ...
                try:
                    if resp.status_code in (404, 405):
                        raise RuntimeError(f"Endpoint not found: {url} status {resp.status_code}")
                    resp.raise_for_status()
                    j = resp.json()
                    text = None
                    if isinstance(j, dict):
                        text = j.get('text') or j.get('transcription')
                        if not text and 'data' in j and isinstance(j['data'], list) and j['data']:
                            text = j['data'][0].get('text')
                    if not text:
                        raise RuntimeError("Unexpected Groq transcription response")
                    return text
                except Exception as e:
                    try:
                        j = resp.json()
                        err = None
                        if isinstance(j, dict):
                            err = j.get('error') or j.get('message') or j.get('error_message')
                            if isinstance(err, str) and 'Unknown request URL' in err:
                                raise RuntimeError("Unknown request URL")
                    except Exception:
                        pass
                    raise
...
def lang_cb(call):
    ...
    try:
        try:
            ...
            progress_event = threading.Event()
            progress_thread = threading.Thread(target=_progress_updater_thread, args=(chat_id, progress_msg.message_id, progress_event, "Transcribing"), daemon=True)
            ...
            bar_empty = "░"
            try:
                progress_msg = bot.send_message(chat_id, f"Transcribing: 0% [{bar_empty * bars}]")
                progress_event = threading.Event()
                progress_thread = threading.Thread(target=_progress_updater_thread, args=(chat_id, progress_msg.message_id, progress_event, "Transcribing"), daemon=True)
                progress_thread.start()
                progress_start = time.time()
...
def handle_media(message):
    ...
    if last and now - last < COOLDOWN_SECONDS:
        bot.reply_to(message, f"Don't spam 😬")
        return
    ...
    try:
        ...
        try:
            try:
                ...
                progress_event = threading.Event()
                progress_thread = threading.Thread(target=_progress_updater_thread, args=(message.chat.id, progress_msg.message_id, progress_event, "Transcribing"), daemon=True)
                progress_thread.start()
                progress_start = time.time()
            except:
                bars = 12
                bar_empty = "░"
                try:
                    progress_msg = bot.send_message(message.chat.id, f"Transcribing: 0% [{bar_empty * bars}]")
                    progress_event = threading.Event()
                    progress_thread = threading.Thread(target=_progress_updater_thread, args=(message.chat.id, progress_msg.message_id, progress_event, "Transcribing"), daemon=True)
                    progress_thread.start()
                    progress_start = time.time()
...
def send_long_text(chat_id, text, reply_id, uid, action="Transcript"):
    mode = user_mode.get(uid, DEFAULT_MODE)
