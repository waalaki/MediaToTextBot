...
def transcribe_media_groq(file_url, mime_type, language_code):
    ...
    try:
        ...
        need_m4a = not mime_type in ("audio/mp4", "audio/x-m4a", "audio/m4a") and not tmp_in.lower().endswith(".m4a")
        use_path = tmp_in
        use_mime = mime_type or "application/octet-stream"
        if need_m4a:
            fd2, tmp_out = tempfile.mkstemp(suffix=".m4a")
            os.close(fd2)
            try:
                subprocess.run(["ffmpeg", "-y", "-i", tmp_in, "-c:a", "aac", "-b:a", "192k", "-ar", "44100", tmp_out], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=60)
                use_path = tmp_out
                use_mime = "audio/mp4"
            except Exception:
                use_path = tmp_in
        ...
        def perform(key):
            with open(use_path, "rb") as audio_file:
                files = {'file': ('audio.m4a', audio_file, use_mime)}
