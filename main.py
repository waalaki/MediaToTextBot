...
def convert_video_to_wav(input_path: str) -> str:
    ...
    command = [FFMPEG_BINARY, "-i", input_path, "-vn", "-acodec", "pcm_s16le", "-ac", "1", "-ar", "16000", output_path, "-y"]
...
def upload_and_transcribe_gemini(file_path: str, key: str, media_type: str) -> str:
    original_path = file_path
    converted_path = None
    ext = os.path.splitext(file_path)[1].lower()
    video_extensions = [".mp4", ".mov", ".mkv", ".webm", ".avi", ".flv", ".m4v"]
    if media_type and media_type.startswith("video") or ext in video_extensions:
        converted_path = convert_video_to_wav(file_path)
...
def handle_media(message):
    ...
    try:
        ...
        with open(file_path, 'wb') as f:
            f.write(downloaded)
        text = upload_and_transcribe_gemini(file_path, user_key, media_type)
        if not text:
