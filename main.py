...
async def handle_media(client, message):
    ...
    try:
        ...
        loop = asyncio.get_event_loop()
        converted = await loop.run_in_executor(None, convert_to_opt_mp3, d_path)
        text = await loop.run_in_executor(None, upload_and_transcribe_gemini, converted, user_key, message.from_user.id)
        if not text:
        ...
        try:
            if os.path.exists(file_path):
                try:
                    ...
                    pass
            base_opt = os.path.splitext(file_path)[0] + "_opt.mp3"
            if os.path.exists(base_opt):
                try:
                    os.remove(base_opt)
                except:
                    pass
