...
def transcribe_media_groq(file_url, mime_type, language_code):
    ...
    try:
        ...
        need_wav = not mime_type in ("audio/wav", "audio/x-wav", "audio/wave") and not tmp_in.lower().endswith(".wav")
        use_path = tmp_in
        ...
        if need_wav:
            ...
            os.close(fd2)
            try:
                subprocess.run(["ffmpeg", "-y", "-i", tmp_in, "-ar", "16000", "-ac", "1", "-f", "wav", tmp_out], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=60)
                use_path = tmp_out
                use_mime = "audio/wav"
            except Exception:
                use_path = tmp_in
        ...
        def perform(key):
            ...
            with open(use_path, "rb") as audio_file:
                files = {'file': ('audio.wav', audio_file, use_mime)}
                data = {'model': 'whisper-large-v3'}
                if language_code:
                    data['language'] = language_code
                headers = {'Authorization': f'Bearer {key}'}
                endpoints = ['https://api.groq.com/openai/v1/audio/transcriptions', 'https://api.groq.com/v1/audio/transcriptions']
                last_exc = None
                for url in endpoints:
                    ...
                    try:
                        if resp.status_code in (404, 405):
                            last_exc = RuntimeError(f"Endpoint not found: {url} status {resp.status_code}")
                            continue
                        resp.raise_for_status()
                        j = resp.json()
                        text = None
