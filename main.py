...
def fake_progress_runner(chat_id):
    try:
        first = bot.send_message(chat_id, "Progress: 0%")
        ...
        for p in steps:
            time.sleep(1)
            try:
                bot.edit_message_text("Progress: %s%%" % p, chat_id, first.message_id)
            except:
                pass
        time.sleep(1)
    except Exception:
...
def process_file_worker(chat_id, file_path, orig_msg, lang_param):
    progress_thread = threading.Thread(target=fake_progress_runner, args=(chat_id,), daemon=True)
