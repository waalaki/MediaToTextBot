...
def lang_cb(call):
    ...
    try:
        text = transcribe_with_speechrecognition(file_path, language=code, chunk_seconds=MAX_CHUNK_SECONDS)
        if not text:
...
def export_audio_segment(segment, dest_path):
    segment = segment.set_frame_rate(16000).set_channels(1)
    segment.export(dest_path, format="wav")

def transcribe_with_speechrecognition(file_path, language=None, chunk_seconds=60):
    ...
    try:
        ...
        while index < total_ms:
            start_ms = index
            end_ms = min(index + chunk_ms, total_ms)
            part = audio[start_ms:end_ms]
            part_path = os.path.join(tmpdir, f"chunk_{start_ms}_{end_ms}.wav")
            export_audio_segment(part, part_path)
            with sr.AudioFile(part_path) as source:
                audio_data = recognizer.record(source)
                try:
                    lang_param = None
                    if language:
                        if len(language) == 2:
                            lang_param = f"{language}"
                        else:
                            lang_param = language
                    result = recognizer.recognize_google(audio_data, language=lang_param) if lang_param else recognizer.recognize_google(audio_data)
                    parts.append(result)
...
def handle_media(message):
    ...
    try:
        ...
        if not lang:
            ...
            bot.reply_to(message, "Select the language spoken in your audio or video:", reply_markup=kb)
            return
        text = transcribe_with_speechrecognition(file_path, language=lang, chunk_seconds=MAX_CHUNK_SECONDS)
        if not text:
