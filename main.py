...
GEMINI_RETRY_CYCLES = int(os.environ.get("GEMINI_RETRY_CYCLES", "3"))
...
def execute_gemini_action(action_callback):
    last_exc = None
    for attempt in range(max(1, GEMINI_RETRY_CYCLES)):
        for model in GEMINI_MODEL_FALLBACKS:
            global GEMINI_MODEL
            prev_model = GEMINI_MODEL
            GEMINI_MODEL = model
            logging.info(f"Attempt {attempt+1}/{GEMINI_RETRY_CYCLES} trying Gemini model: {model}")
            keys_to_try = max(1, len(gemini_rotator.keys))
            ...
            GEMINI_MODEL = prev_model
        time.sleep(0.5)
    safe_last = sanitize_exception_message(last_exc) if last_exc else "Unknown error"
    raise RuntimeError(f"Gemini failed after trying models/keys: {safe_last}")
